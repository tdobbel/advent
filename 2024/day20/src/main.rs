...
fn get_next_moves(
    path: &Vec<(i32,i32)>, closed: &mut Vec<(i32,i32)>, walls: &Vec<(i32,i32)>, nx: i32, ny: i32
) -> Vec<Vec<(i32,i32)>> {
...
fn find_shortest_path(
    queue: &mut Vec<Vec<(i32,i32)>>, closed: &mut Vec<(i32,i32)>, walls: &Vec<(i32,i32)>,
    end: (i32,i32), nx: i32, ny: i32
) -> Option<Vec<(i32,i32)>> {
    while queue.len() > 0 {
...
fn count_shortcuts(
    path: &Vec<(i32,i32)>, min_save: usize, n_cheat: usize
) -> HashMap<usize,i32> {
...
fn main() {
    ...
    let path = find_shortest_path(&mut queue, &mut closed, &mut walls, end, nx, ny);
    let path = path.unwrap();
    plot_path(&path, &walls, nx, ny);
    println!("The program can complete the course un {} picoseconds", path.len()-1);
